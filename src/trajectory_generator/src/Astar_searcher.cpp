...
bool Astarpath::AstarSearch(Vector3d start_pt, Vector3d end_pt) {
  ...
  while (!Openset.empty()) {
    ...
    Openset.erase(Openset.begin());//1.弹出g+h最小的节点
    //????
    ...
    AstarGetSucc(currentPtr, neighborPtrSets, edgeCostSets);//3.拓展当前节点
    //????
    //4.填写信息，完成更新
    for(unsigned int i=0;i<neighborPtrSets.size();i++)
    {
      ...
      if(neighborPtr->id==0)
      {
        neighborPtr->id = 1; // 标记为已发现
        neighborPtr->g_score = tentative_g_score; // 设置g分数
        neighborPtr->Father = currentPtr; // 设置父节点
        neighborPtr->f_score = tentative_g_score + getHeu(neighborPtr, endPtr); // 计算f分数
        Openset.insert(make_pair(neighborPtr->f_score, neighborPtr)); // 添加到开放集
        //???
        continue;
      }
